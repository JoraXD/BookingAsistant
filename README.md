...
- Можно попросить: "покажи последние поездки" или "отмени поездку в <город>"

## История поездок

Подтверждённые бронирования сохраняются в базу SQLite `trips.db`. Команда
"покажи последние поездки" выводит список последних записей пользователя,
а фраза вида "отмени поездку в Москву" помечает найденную активную поездку
как отменённую.
